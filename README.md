...
+ Dans un contexte professionnel : ne pas faire des visuels différents pour dire la même chose (comme c'était le cas ici avec le scartter plot et le bar chart + les nuages de mots). Ici, on a exploré différentes visualisations pour une même chose pour des questions pédagogiques (afin de manipuler les outils) mais ce n'est pas à faire en contexte professionnel car comme on dit "Time is money".
